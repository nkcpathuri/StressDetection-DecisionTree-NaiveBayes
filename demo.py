...
selector = SelectKBest(score_func=chi2, k=1000)  # Adjust the value of k as needed
...
print("Accuracy Score with Feature Selection (Multinomial Naive Bayes):", accuracy_score(tahmin, y_test))
...
# Create and train a Decision Tree classifier using the reduced feature set
...
print("Accuracy Score with Feature Selection (Decision Tree):", accuracy_score(y_test, tahmin1))
...
# Transform the user input text into a numerical feature vector using the selected features
user_vector_selected = selector.transform(vect.transform([user]))

# Use the Decision Tree model to predict the label for the user input text
output = d.predict(user_vector_selected)
